...
class TradingConfig:
    ...
    FLIP_THRESHOLD_PARAMS = {
        'ranges': [
            {'range': [0, 0.20], 'buy_ratio': 6.0, 'sell_ratio': 8.0},    # 波动率 0-20%，买入阈值比例7.0，卖出阈值比例9.0
            {'range': [0.20, 0.40], 'buy_ratio': 5.0, 'sell_ratio': 7.0},  # 波动率 20-40%，买入阈值比例6.0，卖出阈值比例8.0
            {'range': [0.40, 0.60], 'buy_ratio': 4.0, 'sell_ratio': 6.0},  # 波动率 40-60%，买入阈值比例5.0，卖出阈值比例7.0
            {'range': [0.60, 0.80], 'buy_ratio': 5.0, 'sell_ratio': 5.0},  # 波动率 60-80%，买入阈值比例4.0，卖出阈值比例6.0
            {'range': [0.80, 1.00], 'buy_ratio': 2.0, 'sell_ratio': 4.0},  # 波动率 80-100%，买入阈值比例3.0，卖出阈值比例5.0
            {'range': [1.00, 1.20], 'buy_ratio': 1.0, 'sell_ratio': 3.0},  # 波动率 100-120%，买入阈值比例2.0，卖出阈值比例4.0
            {'range': [1.20, 999], 'buy_ratio': 0.8, 'sell_ratio': 2.0}    # 波动率 >120%，买入阈值比例1.0，卖出阈值比例3.0
        ],
    ...
    GRID_PARAMS = {
        ...
        'volatility_threshold': {
            'ranges': [
                ...
                {'range': [0.20, 0.40], 'grid': 1.2},  # 波动率 20-40%，网格1.0%（从1.5%改为1.2%）
